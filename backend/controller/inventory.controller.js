...
class InventoryController {
  ...
  async addFlower(req, res) {
    ...
    if (req.file) {
      image = `${req.protocol}://${req.get("host")}/uploadsFlowers/${
        req.file.filename
      }`;
    ...
    try {
      const updatedInventory = await InventoryService.addFlower(
        username,
        flowerData
      );
  ...
  async updateFlower(req, res) {
    ...
    try {
      const updatedInventory = await InventoryService.updateFlower(
        username,
        flowerId,
        updatedData
      );
  ...
  async deleteFlower(req, res) {
    const { username, flowerId } = req.params;
    try {
      const updatedInventory = await InventoryService.deleteFlower(
        username,
        flowerId
      );
      res.status(200).json(updatedInventory);
